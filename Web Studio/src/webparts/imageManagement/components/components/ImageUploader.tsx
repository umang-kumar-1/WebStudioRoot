...
const ImageUploader: React.FC<ImageUploaderProps> = ({
    ...
    <div className="col">
      ...
      >
        <div className="card-body d-flex flex-column justify-content-center align-items-center p-4">
          <div className="mb-2" style={{ fontSize: '2rem' }}>{children}</div>
  ...
  const renderSelectionScreen = () => (
    <div className="text-center">
      <h5 className="mb-4">{getTranslation('MSG_CHOOSE_ADD_METHOD', currentLanguage)}</h5>
      ...
      <div className="row row-cols-2 row-cols-md-4 g-3">
        ...
        >
          <i className="bi bi-clipboard text-primary" />
        </SelectionCard>
        <SelectionCard
        ...
        >
          <i className="bi bi-images text-primary" />
        </SelectionCard>
      </div>
    </div>
  );
  ...
  const BackButton = () => (
    ...
    >
      <span className="siteColor">{getTranslation('BTN_BACK_OPTIONS', currentLanguage)}</span>
    </button>
  );
    ...
    <div style={{ minHeight: "300px" }}>
      ...
      {uploadMethod === "upload" && (
        <div>
          <BackButton />
          <div
            className={`text-center p-5 border-2 rounded-3 ${isDragging
          ...
          >
            <i
              ...
              style={{ fontSize: "4rem" }}
            ></i>
            <h2 className="mt-3">{getTranslation('TITLE_UPLOAD_IMAGE', currentLanguage)}</h2>
            ...
            <label htmlFor="file-upload" className="btn btn-primary mt-3">
              <i className="bi bi-folder2-open me-2"></i>{getTranslation('BTN_CHOOSE_FILE', currentLanguage)}
            </label>
          </div>
        </div>
      )}
      ...
      {uploadMethod === "url" && (
        <div>
          ...
          <div className="d-flex flex-column justify-content-center align-items-center h-100 p-4">
            <h4 className="mb-3">{getTranslation('TITLE_LOAD_FROM_URL', currentLanguage)}</h4>
            <form
                  ...
                  {isLoading ? (
                    <>
                      <span
                        ...
                        aria-hidden="true"
                      ></span>{" "}
                      {getTranslation('LABEL_LOADING', currentLanguage)}...
                    </>
                  ) : (
                    <>
                      <i className="bi bi-arrow-down-circle me-2"></i>{getTranslation('BTN_LOAD', currentLanguage)}
                    </>
                  )}
                </button>
              </div>
      ...
      {uploadMethod === "paste" && (
        ...
        >
          <i
            ...
            style={{ fontSize: "4rem" }}
          ></i>
          <h2 className="mt-3">{getTranslation('TITLE_PASTE_IMAGE', currentLanguage)}</h2>
      ...
      {uploadMethod === "gallery" && (
        <div>
          ...
          <div className="d-flex flex-column justify-content-center h-100">
            <h4 className="mb-3 text-center">{getTranslation('TITLE_SELECT_GALLERY', currentLanguage)}</h4>
            {isLoading && (
              <div className="text-center">
                <div className="spinner-border siteColor" role="status">
                  <span className="visually-hidden">{getTranslation('LABEL_LOADING', currentLanguage)}</span>
                </div>
                <p>{getTranslation('LABEL_LOADING_IMAGE', currentLanguage)}</p>
              </div>
            )}
            {!isLoading && (
              <div
