...
export const getPages = async (): Promise<any[]> => {
    ...
    return await sp.web.lists.getByTitle('SmartPages').items
        .select('Id', 'Title', 'MultilingualTitle', 'Slug', 'PageStatus', 'IsHomePage', 'Description', 'SEOConfig', 'VersionNote', 'Modified', 'Created', 'ImageUrl', 'ImageName', 'SortOrder', 'Author/Title', 'Editor/Title')
...
export const saveContainer = async (data: any): Promise<any> => {
    ...
    return await sp.web.lists.getByTitle('Containers').items.add(payload);
};
...
export const getNavigation = async (): Promise<any[]> => {
    ...
    return await sp.web.lists.getByTitle('TopNavigation').items
        .select('Id', 'Title', 'Parent/Id', 'NavType', 'Status', 'SmartPage/Id', 'SmartPage/Title', 'ExternalURL', 'SortOrder', 'IsVisible', 'OpenInNewTab', 'Translations', 'Modified')
