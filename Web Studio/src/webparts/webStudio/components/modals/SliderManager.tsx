                        ...
                        <div className="space-y-8">
                            ...
                            <div className="bg-white p-6 border border-gray-200 rounded-sm shadow-sm relative">
                                ...
                                <div className="w-full aspect-video bg-gray-100 border border-gray-300 overflow-hidden relative group rounded-sm">
                                    ...
                                    <div className={`absolute p-8 flex flex-col justify-center overflow-y-auto custom-scrollbar ${formState.layout === 'text_overlay' ? 'inset-0 bg-black/40 text-white items-center text-center' : ''} ${formState.layout === 'solid_color' ? 'inset-0 bg-gray-800 text-white items-center text-center' : ''} ${formState.layout === 'split_left_img' ? 'right-0 top-0 bottom-0 w-1/2 bg-white text-gray-800 items-start text-left pl-12' : ''} ${formState.layout === 'split_right_img' ? 'left-0 top-0 bottom-0 w-1/2 bg-white text-gray-800 items-start text-left pr-12' : ''}`}>
...
export const SliderManager = ({ onClose }: { onClose: () => void }) => {
    ...
    const updateSlideData = (id: string, data: any) => {
        ...
        if (isImageTextSlider) {
            ...
            const updatedTranslations: any = existingItem?.translations ? { ...existingItem.translations } : { en: {} };
            ...
            updatedTranslations[currentLanguage].subtitle = data.subtitle || data.layout || '';
            ...
            const spItem: any = {
                ...
                id: id,
                title: data.title || '',
                subtitle: data.subtitle || data.layout || '',
                                ...
                                {isImageTextSlider ? (
                                    <button onClick={async () => {
                                        ...
                                        const spItem: any = {
                                            ...
                                            imageUrl: '',
                                            imageName: '',
                                            translations: { en: { title: 'New Slide', subtitle: '', description: '', ctaText: '' } }
                                        };
                                        const addedItem = await addSliderItem(spItem);
                                        const realId = (addedItem?.id || id) as string;

                                        setSliderContainer(prev => {
                                            if (!prev) return null;
                                            const existingTagged: string[] = prev.settings.taggedItems || [];
                                            if (existingTagged.includes(realId)) return prev;
                                            return { ...prev, settings: { ...prev.settings, taggedItems: [...existingTagged, realId] } };
                                        });

                                        updateSliderSetting('slides', [...slides, {
                                            id: realId, title: 'New Slide', subtitle: '', desc: '', image: '', cta: '', layout: 'text_overlay', url: '', adjustments: { zoom: 1, rotate: 0, brightness: 100, contrast: 100 }
                                        }]);

                                        // Removed setEditingSlideId(id) so it just adds to the list
                                        ...
                                        {taggedSliderItems.map((item) => (
                                            <div key={item.id} className="bg-white border border-gray-200 rounded-sm shadow-sm p-4 flex gap-4 items-center group hover:shadow-md transition-all cursor-default">
                                                ...
                                                <div className="flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                                                    ...
                                                    }} className="p-2 text-gray-400 hover:text-white rounded-sm transition-all hover:bg-[var(--edit-icon-bg,#2563eb)]" title={getTranslation('BTN_EDIT', currentLanguage)}><Pencil className="w-4 h-4" /></button>
