...
import { useStore, getLocalizedText, getItemTranslation, getTranslation, GLOBAL_DEFAULT_IMAGE } from '../../store';
...
const DataGridEditor = ({ data, setData, onClose }: { data: Container, setData: React.Dispatch<React.SetStateAction<Container>>, onClose: () => void }) => {
    const [activeTab, setActiveTab] = useState<'CONTENT' | 'SETTINGS' | 'TRANSLATION'>('CONTENT');
    const {
    ...
    const updateContent = (key: string, val: string) => {
        setData((prev: Container) => ({ ...prev, content: { ...prev.content, [key]: { ...(prev.content[key] as any), [currentLanguage]: val } } }));
    };
    ...
    const updateTranslationContent = (key: string, val: string) => {
        setData((prev: Container) => {
            ...
            if (key === 'subheading' || key === 'description') {
                newSettings[key] = val;
            }
            return { ...prev, content: newContent, settings: newSettings };
        });
    };
    ...
    // Filter Items
    const availableItems = sourceData.filter((item: any) => {
        if (taggedIds.includes(item.id)) return false;
        // Filter out draft Smart Pages specifically as per requirement
        if (source === 'Smart Pages' && item.status !== 'Published') return false;
        if (!searchTerm) return true;
        const title = getItemTranslation(item, currentLanguage, (source === 'Contacts' || source === 'Contact') ? 'fullName' : 'title');
    ...
    const handleTag = async (id: string) => {
        ...
        // Auto-Publish Logic: Skip for Smart Pages as requested
        if (source === 'Smart Pages') return;
    ...
    const handleUntag = async (id: string) => {
        ...
        // Auto-Draft Logic: Skip for Smart Pages as requested
        if (source === 'Smart Pages') return;
    ...
    const handleSaveEdit = (item: any) => {
        ...
        else if (source === 'Smart Pages') updatePage(item);
        setEditingItem(null);
    };
    ...
    return (
        <div className="flex flex-col h-full bg-gray-50">
                                ...
                                <div className="p-4 border-b border-gray-200 bg-gray-50 flex flex-col gap-3">
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            className="w-full border border-gray-300 p-2 pl-8 text-sm rounded-sm focus:border-[var(--primary-color)] outline-none"
                                        />
            ...
            {showCreate && source === 'Smart Pages' && <CreateSmartPageModal onSave={handleCreateInit} onCancel={() => setShowCreate(false)} />}

            {/* Edit Modals (non-ImageSlider) */}
            {editingItem && normalizedSource === 'News' && <NewsEditor item={editingItem} onSave={handleSaveEdit} onCancel={() => setEditingItem(null)} onDelete={(id: string) => setDeleteId(id)} />}
            {editingItem && normalizedSource === 'Event' && <EventEditor item={editingItem} onSave={handleSaveEdit} onCancel={() => setEditingItem(null)} onDelete={(id: string) => setDeleteId(id)} />}
            {editingItem && normalizedSource === 'Document' && <DocumentEditor item={editingItem} onSave={handleSaveEdit} onCancel={() => setEditingItem(null)} onDelete={(id: string) => setDeleteId(id)} />}
            {editingItem && normalizedSource === 'Container Items' && <ContainerItemEditor item={editingItem} onSave={handleSaveEdit} onCancel={() => setEditingItem(null)} onDelete={(id: string) => setDeleteId(id)} />}
            {editingItem && normalizedSource === 'Contact' && <ContactEditor item={editingItem} onSave={handleSaveEdit} onCancel={() => setEditingItem(null)} onDelete={(id: string) => setDeleteId(id)} />}
            {editingItem && normalizedSource === 'ImageSlider' && <SliderItemEditor item={editingItem} onSave={handleSaveEdit} onCancel={() => setEditingItem(null)} onDelete={(id: string) => setDeleteId(id)} />}
            ...
            {deleteId && (
                <ConfirmDeleteDialog
                    ...
                    onConfirm={() => {
                        ...
                        else if (source === 'Smart Pages') deletePage(deleteId);
                        setDeleteId(null);
                        setEditingItem(null);
                    }}
...
const ContactFormEditor = ({ data, setData, onClose }: { data: Container, setData: React.Dispatch<React.SetStateAction<Container>>, onClose?: () => void }) => {
    ...
    const resolvedAlignment = (String(data.settings.alignment || data.settings.align || 'center').toLowerCase() === 'left' ? 'left' : 'center') as 'center' | 'left';
    ...
    const updateTranslationContent = (key: string, val: string) => {
        setData((prev: Container) => {
            ...
            const newSettings: any = { ...prev.settings };
    ...
    const PreviewForm = () => (
        <div
            ...
            style={{
                backgroundColor: resolvedBgType === 'site-color' ? 'var(--bg-body)' : (resolvedBgType === 'color' ? resolvedBgColor : 'white'),
        ...
        >
            {resolvedBgType === 'image' && <div className="absolute inset-0 bg-white/95 backdrop-blur-sm z-0 rounded-sm"></div>}

            <div className={`relative z-10 ${resolvedAlignment === 'left' ? 'text-left' : 'text-center'}`}>
                ...
                {activeTab === 'TEXT' && (
                    <div className="p-8 bg-gray-50 w-full space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        ...
                        <div className="bg-white p-6 border border-gray-200 shadow-sm rounded-sm">
                            ...
                            <div className="grid grid-cols-2 gap-6 mb-4">
                                <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Container Title (Internal)</label><input className="w-full border p-2 text-sm rounded-sm focus:ring-1 focus:ring-[var(--primary-color)] outline-none" value={data.title || ''} onChange={(e) => setData((prev: Container) => ({ ...prev, title: e.target.value }))} placeholder="e.g. Careers Contact Form" /></div>
                            ...
                            <div className="grid grid-cols-2 gap-6 mb-4">
                                ...
                                <VisualSelector label="Alignment" value={resolvedAlignment} onChange={(v: any) => updateSetting('alignment', v)} options={[{ value: 'center', label: 'Center', icon: AlignCenter }, { value: 'left', label: 'Left', icon: AlignLeft }]} />
                            </div>
                            <div className="mb-0">
                        ...
                        <div className="bg-white p-6 border border-gray-200 shadow-sm rounded-sm">
                            ...
                            <div className="flex gap-4 mb-4">
                                <VisualOption label="None" icon={X} active={resolvedBgType === 'none'} onClick={() => updateSetting('bgType', 'none')} />
                                <VisualOption label="Color" icon={Palette} active={resolvedBgType === 'color'} onClick={() => updateSetting('bgType', 'color')} />
                                <VisualOption label="Site Color" icon={Palette} active={resolvedBgType === 'site-color'} onClick={() => updateSetting('bgType', 'site-color')} />
                                <VisualOption label="Image" icon={ImageIcon} active={resolvedBgType === 'image'} onClick={() => updateSetting('bgType', 'image')} />
                            </div>
...
// --- 4. TABLE EDITOR ---
const TableEditor = ({ data, setData, onClose }: { data: Container, setData: React.Dispatch<React.SetStateAction<Container>>, onClose: () => void }) => {
    const [activeTab, setActiveTab] = useState<'CONTENT' | 'SETTINGS' | 'TRANSLATION'>('CONTENT');
    const { currentLanguage } = useStore();
    ...
    const updateContent = (key: string, val: any) => {
        setData((prev: Container) => ({ ...prev, content: { ...prev.content, [key]: val } }));
    };
                        ...
                        <div className="mx-auto bg-white p-8 border border-gray-200 shadow-sm rounded-sm space-y-8">
                            <div className="grid grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input className="w-full border border-gray-300 p-2.5 text-sm rounded-sm focus:ring-1 focus:ring-[var(--primary-color)] outline-none" value={data.title || ''} onChange={(e) => setData((prev: Container) => ({ ...prev, title: e.target.value }))} placeholder="e.g. Project Phases Table" />
                                </div>
                                <div>
...
// --- 5. MAP EDITOR ---
const MapEditor = ({ data, setData, onClose }: { data: Container, setData: React.Dispatch<React.SetStateAction<Container>>, onClose: () => void }) => {
