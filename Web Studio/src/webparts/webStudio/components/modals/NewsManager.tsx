...
import { useStore, getItemTranslation, getTranslation, GLOBAL_DEFAULT_IMAGE } from '../../store';
import { translateText } from '../../services/geminiService';
import { NewsItem } from '../../types';
import { GenericModal, ConfirmDeleteDialog, EditTrigger } from './SharedModals';
...
import {
    ...
    Calendar, Upload, ChevronDown, ArrowUpAZ, ArrowDownAZ
} from 'lucide-react';
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in zoom-in-95 duration-200">
            <div className="bg-white w-[500px] shadow-2xl rounded-sm border border-gray-300 flex flex-col overflow-hidden">
                <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-white">
                    <h3 className="text-lg font-bold text-[var(--primary-color)]">{getTranslation('TITLE_CREATE_NEWS', currentLanguage)}</h3>
...
export const NewsEditor = ({ item, onSave, onCancel, onDelete }: any) => {
    ...
    const [formData, setFormData] = useState<any>({
        ...
        imageName: item.imageName || 'NavLogo',
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in zoom-in-95 duration-200">
            <div className="bg-white w-[1000px] shadow-2xl rounded-sm border border-gray-300 flex flex-col max-h-[90vh] overflow-hidden">
                ...
                <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-white flex-shrink-0">
                    <h3 className="text-xl font-bold text-[var(--primary-color)]">{getTranslation('TITLE_EDIT_NEWS', currentLanguage)} - {formData.title}</h3>
                ...
                <div className="flex border-b border-gray-200 px-6 bg-white flex-shrink-0 gap-6">
                    {[
                        { id: 'BASIC', label: getTranslation('TAB_BASIC_INFO_NEWS', currentLanguage) },
                        { id: 'IMAGE', label: getTranslation('TAB_IMAGE_INFO', currentLanguage) },
                        { id: 'SEO', label: getTranslation('TAB_SEO', currentLanguage) },
                        { id: 'TRANSLATION', label: getTranslation('TAB_TRANSLATION_NEWS', currentLanguage) }
                    ].map(tab => (
                        <button
                            key={tab.id}
                    ...
                    {activeTab === 'BASIC' && (
                        <div className="space-y-6 bg-white p-6 border border-gray-200 shadow-sm rounded-sm">
                            <div className="grid grid-cols-2 gap-6">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">{getTranslation('LABEL_PUBLISHING_DATE', currentLanguage)}</label>
                                    <div className="relative">
                                        <input type="date" className="w-full border border-gray-300 p-2.5 text-sm rounded-sm focus:ring-1 focus:ring-[var(--primary-color)] outline-none" value={formData.publishDate} onChange={e => updateField('publishDate', e.target.value)} />
...
export const NewsManager = ({ onClose }: any) => {
    ...
    const handleCreateInit = async (title: string) => {
        ...
        const addedItem = await addNews(newItem);
        setShowCreate(false);
    ...
    return (
        <GenericModal
            ...
            headerIcons={
                <div className="flex items-center gap-4">
                    <span className="text-xs text-gray-500 mr-2 hidden md:inline">{getTranslation('MSG_NEWS_DESC', currentLanguage)} <EditTrigger labelKey="MSG_NEWS_DESC" /></span>
        ...
        >
                                        ...
                                        <div className="p-4 flex-1 flex flex-col justify-between">
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 text-xs text-gray-500 mb-2">
                                                    <Calendar className="w-3 h-3" /> {new Date(n.publishDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
                                                </div>
                            ...
                            <table className="w-full text-sm text-left">
                                <thead className="bg-gray-100 text-gray-600 border-b border-gray-200 text-xs font-bold uppercase tracking-wider">
                                    <tr>
                                        ...
                                        <th className="p-4">Title & Date</th>
                                    ...
                                    {filteredNews.map((n: any) => (
                                        <tr key={n.id} className="hover:bg-[var(--brand-light)] transition-colors group">
                                            ...
                                            <td className="p-3">
                                                ...
                                                <div className="text-xs text-gray-500 flex items-center gap-1"><Calendar className="w-3 h-3" /> {new Date(n.publishDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}</div>
                                            </td>
                                            <td className="p-3">
            ...
            {deleteId && <ConfirmDeleteDialog title="Delete News" message="Are you sure you want to delete this article? This action cannot be undone." onConfirm={() => handleDelete(deleteId)} onCancel={() => setDeleteId(null)} />}
        </GenericModal>
    );
};
