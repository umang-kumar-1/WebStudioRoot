...
const ContactFormRenderer = ({ config }: { config: ContactConfigType }) => {
    ...
    return (
        <div
            className="w-full p-10 border border-gray-200 rounded-sm relative transition-all duration-300 mx-auto"
            style={{
        ...
        >
            ...
            <div className="relative z-10 bg-white p-10 w-full max-w-xl shadow-xl border border-gray-200 rounded-sm mx-auto">
                <div className={`${resolvedAlignment === 'center' ? 'text-center' : 'text-left'} mb-8`}>
                            ...
                            <div className="w-full space-y-8 animate-in slide-in-from-left-4 duration-300">
                                ...
                                <div className="bg-white p-6 border border-gray-200 shadow-sm rounded-sm">
                                    ...
                                    <div className="grid grid-cols-2 gap-6 mb-4">
                                        <div>
                                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Heading</label>
                                            <input className="w-full border border-gray-300 p-2 text-sm rounded-sm" value={config.heading} onChange={(e) => setConfig(prev => ({ ...prev, heading: e.target.value }))} placeholder="Form Title" />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Subheading</label>
                                            <input className="w-full border border-gray-300 p-2 text-sm rounded-sm" value={config.subheading} onChange={(e) => setConfig(prev => ({ ...prev, subheading: e.target.value }))} placeholder="Optional Subtitle" />
                                        </div>
                                    </div>
                                    <div className="mb-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-4">
                                            {['Center', 'Left'].map(align => (
                                                <button
                                                    key={align}
                                                    onClick={() => setConfig(prev => ({ ...prev, alignment: align.toLowerCase() as ContactConfigType['alignment'] }))}
                                ...
                                <div className="bg-white p-6 border border-gray-200 shadow-sm rounded-sm">
                                    ...
                                    <div className="flex gap-2 mb-4">
                                        <button onClick={() => setConfig(prev => ({ ...prev, bgType: 'none' }))} className="flex-1 py-2 text-sm font-bold border rounded-sm transition-colors" style={{ backgroundColor: config.bgType === 'none' ? themeConfig['--primary-color'] : '#fff', color: config.bgType === 'none' ? '#fff' : '#374151' }}>No Background</button>
                                        <button onClick={() => setConfig(prev => ({ ...prev, bgType: 'color' }))} className="flex-1 py-2 text-sm font-bold border rounded-sm transition-colors" style={{ backgroundColor: config.bgType === 'color' ? themeConfig['--primary-color'] : '#fff', color: config.bgType === 'color' ? '#fff' : '#374151' }}>Color</button>
                                        <button onClick={() => setConfig(prev => ({ ...prev, bgType: 'site-color' }))} className="flex-1 py-2 text-sm font-bold border rounded-sm transition-colors" style={{ backgroundColor: config.bgType === 'site-color' ? themeConfig['--primary-color'] : '#fff', color: config.bgType === 'site-color' ? '#fff' : '#374151' }}>Site Color</button>
                                        <button onClick={() => setConfig(prev => ({ ...prev, bgType: 'image' }))} className="flex-1 py-2 text-sm font-bold border rounded-sm transition-colors" style={{ backgroundColor: config.bgType === 'image' ? themeConfig['--primary-color'] : '#fff', color: config.bgType === 'image' ? '#fff' : '#374151' }}>Image</button>
                                    </div>
                                    {config.bgType === 'site-color' && (
                                        <div className="text-sm text-gray-600">
                                            Uses site background color from Theme settings.
                                        </div>
                                    )}
                                    {config.bgType === 'color' && (
                                        <div className="flex items-center gap-2">
                                ...
                                <div className="bg-white p-6 border border-gray-200 shadow-sm rounded-sm">
                                    ...
                                    <input className="w-full border p-2 text-sm rounded-sm" value={config.buttonText} onChange={(e) => setConfig(prev => ({ ...prev, buttonText: e.target.value }))} placeholder="Send Message" />
                                </div>
...
export const SelectHeaderTemplatePopup = ({ pageId, onClose }: { pageId: string, onClose: () => void }) => {
    const { addContainer, updateContainer, pages, themeConfig } = useStore();
    ...
    const [contactConfig, setContactConfig] = useState<ContactConfigType>(DEFAULT_CONTACT_CONFIG);
    ...
    }, [selectedCategory, existingContactContainer?.id]);
    ...
    const handleGlobalSave = async () => {
        ...
        if (selectedCategory === 'HEADER' && selectedHeaderId) {
            ...
            if (existingContactContainer) {
                newContainer = {
                    ...
                    settings: {
                        ...contactConfig,
                        // Keep aliases for backward compatibility with older saved settings.
                        align: contactConfig.alignment,
                        backgroundColor: contactConfig.bgType === 'site-color' ? 'site-color' : contactConfig.bgColor,
                        backgroundImage: contactConfig.bgImage
                    },
                    content: {
                        ...(existingContactContainer.content || {}),
                        title: { en: contactConfig.heading, de: contactConfig.heading, fr: contactConfig.heading, es: contactConfig.heading }
                    }
                };
        ...
        if (newContainer) {
            if (selectedCategory === 'CONTACT_FORM' && shouldUpdateExistingContact) {
                await updateContainer(pageId, newContainer);
            } else {
                await addContainer(pageId, newContainer);
            }
            onClose();
        }
    };
