...
export const ContainerItemEditor = ({ item, onSave, onCancel, onDelete }: any) => {
    ...
    const [formData, setFormData] = useState<any>({
        ...
        translations: item.translations || { en: { title: '', description: '' } },
    });
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in zoom-in-95 duration-200">
            <div className="bg-white w-[1000px] shadow-2xl rounded-sm border border-gray-300 flex flex-col max-h-[90vh] overflow-hidden">
                ...
                <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-white flex-shrink-0">
                    <h3 className="text-xl font-bold text-[var(--primary-color)]">{getTranslation('TITLE_EDIT_CONTAINER_ITEM', currentLanguage)} - {formData.title}</h3>
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <input type="number" className="w-full border border-gray-300 p-2.5 text-sm rounded-sm focus:ring-1 focus:ring-[var(--primary-color)] outline-none" value={formData.sortOrder} onChange={e => updateField('sortOrder', parseInt(e.target.value))} />
                            ...
                            <div className="border-t border-gray-200 pt-6">
                                <div className="flex gap-6 border-b border-gray-200 mb-4">
                                    {['COPY & PASTE', 'UPLOAD', 'CHOOSE FROM EXISTING'].map(sub => (
                                        ...
                                        >
                                            {sub === 'CHOOSE FROM EXISTING' ? `${sub} (${images.length})` : sub}
                                        </button>
                                    ))}
                                </div>
                                                    ...
                                                    <div
                                                        ...
                                                        className={`
                                                            ...
                                                            ${formData.imageUrl === img.url
                                                                ? 'border-b-[var(--primary-color)] ring-2 ring-[var(--primary-color)] ring-opacity-20 z-10'
...
export const ContainerItemManager = ({ onClose }: any) => {
    ...
    return (
        <GenericModal
            ...
            headerIcons={
                <div className="flex items-center gap-4">
                    <span className="text-xs text-gray-500 mr-2 hidden md:inline">{getTranslation('MSG_CONTAINER_ITEM_DESC', currentLanguage)} <span className="text-blue-400 border border-blue-200 rounded-full w-4 h-4 inline-flex items-center justify-center text-[10px]">i</span></span>
        ...
        >
                                    ...
                                    <div className="flex h-full">
                                        ...
                                        <div className="p-4 flex-1 flex flex-col justify-between">
                                            ...
                                            <div className="flex justify-end gap-2 pt-2 border-t border-gray-100 opacity-60 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => setEditingItem(n)} className="p-1 hover:bg-blue-50 rounded-sm text-[var(--icon-color)]"><Pencil className="w-3 h-3" /></button>
                                        ...
                                        <tr key={n.id} className="hover:bg-[var(--brand-light)] transition-colors group">
                                            ...
                                            <td className="p-3 text-right">
                                                <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <button onClick={() => setEditingItem(n)} className="p-1.5 hover:bg-blue-50 rounded-sm text-[var(--icon-color)]"><Pencil className="w-4 h-4" /></button>
            ...
            {deleteId && <ConfirmDeleteDialog title="Delete Item" message="Are you sure you want to delete this item? This action cannot be undone." onConfirm={() => handleDelete(deleteId)} onCancel={() => setDeleteId(null)} />}
        </GenericModal>
    );
};
