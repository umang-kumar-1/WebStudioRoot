...
import { ContainerType, Page, NavItem, MultilingualText } from '../../types';
...
import { CreateSmartPageModal } from './SmartPageManager';
...
export const SiteManager = ({ onClose }: { onClose: () => void }) => {
    ...
    // Derived Data
    const internalNavItems = siteConfig.navigation.filter(n => pages.some(p => p.id === n.pageId));
    ...
    const handleCreatePage = (pageData: Partial<Page>) => {
        const newPage: Page = {
            id: `p_${Date.now()}`,
            ...pageData,
            title: pageData.title as MultilingualText,
            slug: pageData.slug || '',
            status: 'Draft',
            createdBy: 'Admin',
            modifiedBy: 'Admin',
                                    ...
                                    <div className="bg-white border border-gray-200 rounded-sm shadow-sm overflow-hidden">
                                        ...
                                        {isPageInfoOpen && (
                                            <div className="p-6 space-y-4">
                                                ...
                                                <div className="grid grid-cols-[140px_1fr] gap-4 items-center">
                                                    <span className="text-sm font-bold text-gray-800 flex items-center gap-1">Containers Status:</span>
                                                    <span className="text-xs text-gray-500 font-medium">
                                                        {activePage.containers.filter(c => c.status === 'Published').length} Published / {activePage.containers.filter(c => c.status === 'Draft').length} Draft
                                                    </span>
                                                </div>
                                                <div className="grid grid-cols-[140px_1fr] gap-4 items-center">
                                                        ...
                                                        if (isPageDraft || hasDraftContainers) {
                                                            return (
                                                                ...
                                                                >
                                                                    ...
                                                                    {isPageDraft ? getTranslation('BTN_PUBLISH', currentLanguage) : 'Publish Changes'}
                                                                </button>
                                                            );
                                                        }
                                                        return null;
