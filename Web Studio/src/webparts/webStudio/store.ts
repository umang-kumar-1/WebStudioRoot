  ...
  (set, get) => ({
    ...
    siteConfig: {
      ...
      footer: {
        ...
        fontSettings: { headingSize: '18px', subHeadingSize: '15px' },
        columns: [
    ...
    addNavItem: async (item) => {
      try {
        ...
        const newItem = { ...item, id: String(realId), status };
    ...
    updateNavItem: async (item) => {
      try {
        const state = get();
        // Dynamic Status: inherit from page if type is Page
        ...
        set((state) => ({ siteConfig: { ...state.siteConfig, navigation: state.siteConfig.navigation.map(n => n.id === item.id ? { ...item, status } : n) } }));
      } catch (error) {
...
export const getItemTranslation = (item: any, lang: LanguageCode, field: string | string[]): string => {
  ...
  const getFromField = (f: string): any => {
    // 1. Direct MultilingualText Check
    if (item[f] && typeof item[f] === 'object' && !Array.isArray(item[f]) && ('en' in item[f] || 'de' in item[f])) {
      return getLocalizedText(item[f], lang);
    }

    // 2. Translations Object Style
    if (item.translations && item.translations[lang]) {
      if (typeof item.translations[lang] === 'string' && f === 'title') return item.translations[lang];
      if (item.translations[lang][f]) return item.translations[lang][f];
    }

    // 3. Fallback to English translation
    if (item.translations && item.translations['en']) {
      if (typeof item.translations['en'] === 'string' && f === 'title') return item.translations['en'];
      if (item.translations['en'][f]) return item.translations['en'][f];
    }
