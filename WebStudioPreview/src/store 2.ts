  ...
  persist(
    (set, get) => ({
      ...
      loadFromApi: async () => {
        try {
          set({ isLoading: true });
          // PHP endpoints using direct file names (cPanel friendly)
          // Example:
          //   axios.post(`${SiteUrl}/HHHHPUBLICAPI/getDataAll.php`, payload)
          const API_ROOT = "https://webstudio.hochhuth-consulting.de/PHPAPISHAREPOINT";
          const API_BATCH = `${API_ROOT}/batch.php`;
          const VIEW_FILE_ROOT = `${API_ROOT}/view-file.php`;

          // 1️⃣ Fetch everything in one batch call (single PHP file)
          const batchData = await fetch(API_BATCH).then(r => r.json()).catch(() => ({}));
          ...
          const allImagesRaw = rawImages.filter((i: any) => i.type !== "folder").map((i: any) => ({
            ...i,
            url: i.url || `${VIEW_FILE_ROOT}?type=images&id=${i.id}`,
            thumbnail: i.thumbnail || i.url || `${VIEW_FILE_ROOT}?type=images&id=${i.id}`
          }));

          const spDocs = rawDocs.filter((i: any) => i.type !== "folder").map((i: any) => ({
            ...i,
            url: i.url || `${VIEW_FILE_ROOT}?type=documents&id=${i.id}`,
            thumbnail: i.thumbnail || i.url || `${VIEW_FILE_ROOT}?type=documents&id=${i.id}`
          }));
