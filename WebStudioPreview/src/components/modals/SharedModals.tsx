        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white w-[900px] shadow-2xl rounded-sm border border-gray-300 flex flex-col max-h-[85vh]">
                <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-white flex-shrink-0">
                    ...
                    <button onClick={onClose} className="text-gray-400 hover:text-gray-600 transition-colors"><X className="w-5 h-5" style={{ color: 'var(--icon-color)' }} /></button>
                </div>
                <div className="flex-1 overflow-auto p-0 bg-white">
                    <table className="w-full text-sm text-left border-collapse">
                        <thead className="bg-white text-[var(--primary-color)] font-bold border-b border-gray-200 sticky top-0 z-10">
                            <tr>
                                <th className="px-6 py-3 w-16 border-r border-gray-100">{getTranslation('TH_NO', currentLanguage) || 'No'}</th>
                                <th className="px-6 py-3 border-r border-gray-100">{getTranslation('LABEL_INFO', currentLanguage) || 'Info'}</th>
                                <th className="px-6 py-3 w-64">{getTranslation('LABEL_MODIFIED_BY', currentLanguage) || 'Modified by'}</th>
                            </tr>
                        </thead>
                                ...
                                <tr key={i} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 align-top border-r border-gray-100">
                                        <div className="space-y-2">
                                            ...
                                            )) : <span className="text-gray-400 italic">{getTranslation('MSG_NO_DETAILS', currentLanguage) || 'No details recorded'}</span>}
                                        </div>
                                    </td>
                ...
                <div className="px-6 py-4 border-t border-gray-200 bg-white flex justify-end flex-shrink-0">
                    <button onClick={onClose} className="px-6 py-2 border border-gray-300 bg-white text-gray-700 text-sm font-bold hover:bg-gray-100 rounded-sm shadow-sm transition-colors">{getTranslation('BTN_CLOSE', currentLanguage)}</button>
                </div>
            </div>
        </div>,
        document.body
...
export const GenericModal = memo(({ title, subtitle, children, onClose, width = 'w-full max-w-2xl', heightClass = 'max-h-[90vh]', noFooter = false, headerIcons = null, className = '', customFooter = null, hasActiveSubModal = false }: any) => {
    const { currentLanguage } = useStore();
    return (
        <div className={`bg-white rounded-sm shadow-2xl flex flex-col ${heightClass} ${width} relative animate-in fade-in zoom-in-95 duration-200 z-50 border border-gray-300 ${className}`}>
            ...
            {!noFooter && !customFooter && (
                <div className="px-6 py-4 border-t border-gray-200 bg-white flex justify-end gap-3 rounded-b-sm flex-shrink-0 z-10">
                    <button onClick={onClose} className="px-6 py-2 border border-gray-300 bg-white text-gray-700 rounded-sm hover:bg-gray-50 text-sm font-bold transition-colors">{getTranslation('BTN_CANCEL', currentLanguage)}</button>
                    <button onClick={onClose} className="px-6 py-2 bg-[var(--primary-color)] text-white rounded-sm hover:opacity-90 text-sm font-bold shadow-sm transition-colors">{getTranslation('BTN_SAVE_CHANGES', currentLanguage)}</button>
                </div>
            )}
            {customFooter && (
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white w-[450px] shadow-2xl rounded-sm border border-gray-300 flex flex-col overflow-hidden">
                ...
                <div className="flex border-t border-gray-200 bg-gray-50">
                    <button onClick={onCancel} className="flex-1 py-4 text-sm font-bold text-gray-700 hover:bg-white transition-colors border-r border-gray-200">{getTranslation('BTN_CANCEL', currentLanguage)}</button>
            ...
            <div className="bg-white w-[700px] shadow-2xl rounded-sm border border-gray-300 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200 relative">
                ...
                <div className="p-8 overflow-y-auto space-y-6 text-sm text-gray-700 flex-1">
                    <section>
                        ...
                        <p>{getTranslation('MSG_HELP_WELCOME', currentLanguage) || 'Welcome to the Management Module. Use this tool to organize your content effectively.'}</p>
                    </section>
                </div>
                <div className="p-4 border-t border-gray-200 flex justify-end flex-shrink-0 bg-white">
                    <button onClick={onClose} className="px-6 py-2 bg-white border border-gray-300 hover:bg-gray-50 text-sm font-bold rounded-sm">{getTranslation('BTN_CLOSE', currentLanguage)}</button>
                </div>
            </div>
        </div>
    );
